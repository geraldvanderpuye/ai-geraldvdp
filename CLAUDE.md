...
## Open work (in order)
0. **Deploy the AI test (next task for Claude Code).** The six-question AI-test rebuild is done and committed locally as `5e08609` on `main` ("Rebuild the AI test: six-question staged diagnostic"), implemented per `AI-TEST-CLAUDE-BRIEF.md` and fully tested (all four result paths, both scoring guardrails, Back/retake, keyboard, no-JS fallback). It could not be pushed from the Cowork session (no GitHub credentials in its sandbox). To deploy: `git push origin main`, then confirm the Vercel **gvdp-site** production deployment goes green and www.gvdp.co.uk serves the new test. Also delete `../_to_delete_gitlock/` (stale git lock/tmp files the sandbox could move but not delete).
0b. Follow-ups after deploy: the three article URLs are placeholder constants in `index.html` (`LEVEL_ONE/TWO/THREE_ARTICLE_URL`, all `#`) until the articles are written; and `api/ask.js` normalises unknown topics to "question", so the test's `assessment-level-0..3` topic values reach the inbox as plain questions until its topic mapping is extended (small, safe change: pass through `assessment-level-*` in the topic line and email subject).
